...
```bash
grep -q "owe-skill" $HOME/.claude/CLAUDE.md 2>/dev/null || cat >> $HOME/.claude/CLAUDE.md << 'OWEEOF'

# OWE — Once Was Enough

All'inizio di ogni sessione, prima di rispondere:

1. Controlla se `$HOME/.owe/index.json` esiste:
   - Non esiste → di' all'utente: "[OWE] Database non trovato. Esegui in un terminale: `python $HOME/.claude/skills/owe-skill/scripts/census.py` (PowerShell: `python $HOME\.claude\skills\owe-skill\scripts\census.py`) — poi riavvia la sessione."
   - Esiste → esegui `python $HOME/.claude/skills/owe-skill/scripts/verify.py --status` e riporta l'output come `[OWE] <output>`
...
[OWE] Installazione completata. Ora esegui in un terminale:
  Git Bash / Mac / Linux : python $HOME/.claude/skills/owe-skill/scripts/census.py
...
Verifica se `$HOME/.owe/index.json` esiste.
...
Leggi `$HOME/.owe/prefs.json` e tieni le preferenze in context per tutta la sessione.
...
2. Esegui: `python $HOME/.claude/skills/owe-skill/scripts/search.py <keyword1> [keyword2] ...`
...
2. Se sì, esegui:
   ```
   python $HOME/.claude/skills/owe-skill/scripts/census.py --add <path_assoluto> <nome> <linea> "<docstring>"
   ```

Oppure aggiungi direttamente in `$HOME/.owe/index.json` → `code.components`:
```json
{
...
2. Se sì:
   - Crea `$HOME/.owe/knowledge/<dominio>/` se non esiste
   - Edita (o crea) `$HOME/.owe/knowledge/<dominio>/notes.json`:
...
```
   ...
   - Aggiorna `$HOME/.owe/index.json` → `knowledge.domains.<dominio>`:
```json
{
  "count": 1,
...
| Comando | Azione |
|---|---|
| `/owe-autosync-on` | `python $HOME/.claude/skills/owe-skill/scripts/census.py --autosync-on` |
| `/owe-autosync-off` | `python $HOME/.claude/skills/owe-skill/scripts/census.py --autosync-off` |
| `/owe-sync` | `python $HOME/.claude/skills/owe-skill/scripts/census.py` |
| `/owe-setup` | `python $HOME/.claude/skills/owe-skill/scripts/census.py --setup` |
| `/owe-status` | `python $HOME/.claude/skills/owe-skill/scripts/verify.py --status` |
| `/owe-pref` | Chiedi la preferenza → aggiungi a `$HOME/.owe/prefs.json` (con conferma) |
| `/owe-export` | Zip `$HOME/.owe/` → Desktop come `owe-backup-<YYYY-MM-DD>.zip` |
| `/owe-import` | Chiedi file → decomprimi in `$HOME/.owe/` (con conferma, sovrascrive) |
...
```bash
# Chiedi conferma prima — sovrascrive $HOME/.owe/
unzip -o <file> -d ~/
```
...
→ Solo se conferma, aggiungi a `$HOME/.owe/prefs.json`.

---

## Struttura $HOME/.owe/prefs.json
